...
function AddressField({
  ...
  const showSuggestions = !picked && suggestions.length > 0;
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        ...
        {recommended && (
          <span className="font-inter text-[10px] uppercase tracking-wider text-zinc-400">
            recommended
          </span>
        )}
        {optional && (
      ...
      {/* Wrapper is `relative` so the absolute-positioned suggestion popover
          floats over subsequent fields rather than pushing them down. */}
      <div className="relative">
        ...
        {showSuggestions && (
          <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-56 overflow-y-auto rounded-lg border border-zinc-200 bg-white p-1 shadow-md">
            {suggestions.map((s, i) => (
              <button
                key={`${s.lat}-${s.lng}-${i}`}
                type="button"
